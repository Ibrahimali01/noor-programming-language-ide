...
import { generateLiveDocs } from "./src/noor-compiler.ts";
...
// API: Live Standard Library Parser & Documenter
app.get("/api/stdlib", (req, res) => {
  try {
    const docs = generateLiveDocs();
    res.json(docs);
  } catch (error: any) {
    console.error("Live docs generation endpoint error:", error);
    res.status(500).json({ error: error.message });
  }
});
