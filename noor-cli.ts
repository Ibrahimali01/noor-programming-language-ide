    ...
    <script>
        window.addEventListener('DOMContentLoaded', function() {
            ...
            const createScene = function() {
                ...
                // الكاميرات الديناميكية الذكية
                let camera;
                if (worldData.cameras && worldData.cameras.length > 0) {
                    const camData = worldData.cameras[0];
                    const camType = (camData['نوع_الكاميرا'] || camData['type'] || 'دوران').trim().toLowerCase();
                    const cx = camData['س'] !== undefined ? camData['س'] : 0;
                    const cy = camData['ص'] !== undefined ? camData['ص'] : 15;
                    const cz = camData['ع'] !== undefined ? camData['ع'] : 30;
                    
                    if (camType.includes('بطل') || camType.includes('fps')) {
                        // الكاميرا ثنائية الأبعاد أو منظور الشخص الأول
                        camera = new BABYLON.UniversalCamera("camera", new BABYLON.Vector3(cx, cy, cz), scene);
                        camera.keysUp.push(87); // W
                        camera.keysDown.push(83); // S
                        camera.keysLeft.push(65); // A
                        camera.keysRight.push(68); // D
                    } else if (camType.includes('تتبع') || camType.includes('follow')) {
                        camera = new BABYLON.FollowCamera("camera", new BABYLON.Vector3(cx, cy, cz), scene);
                        camera.radius = camData['مسافة'] || camData['radius'] || 15;
                        camera.heightOffset = camData['ارتفاع'] || camData['heightOffset'] || 4;
                        camera.rotationOffset = camData['زاوية'] || camData['rotationOffset'] || 180;
                    } else {
                        camera = new BABYLON.ArcRotateCamera("camera", camData['الفا'] || camData['alpha'] || -Math.PI / 2, camData['بيتا'] || camData['beta'] || Math.PI / 2.5, camData['مسافة'] || camData['radius'] || 30, new BABYLON.Vector3(cx, cy, cz), scene);
                    }
                } else {
                ...
                const getColor = (c) => {
                    ...
                    const colorsMap = {
                        ...
                        'ذهب': new BABYLON.Color3(0.95, 0.75, 0.15),
                        'فضه': new BABYLON.Color3(0.85, 0.85, 0.85),
                        'نحاس': new BABYLON.Color3(0.72, 0.45, 0.2)
                    };
                ...
                // إضاءة مخصصة بالكامل أو إضاءة أساسية
                if (worldData.lights && worldData.lights.length > 0) {
                    worldData.lights.forEach((lite, lIdx) => {
                        const type = (lite['نوع'] || lite['type'] || 'hemispheric').trim().toLowerCase();
                        const p = new BABYLON.Vector3(lite['س'] !== undefined ? lite['س'] : 0, lite['ص'] !== undefined ? lite['ص'] : 15, lite['ع'] !== undefined ? lite['ع'] : 0);
                        const d = new BABYLON.Vector3(lite['اتجاه_س'] !== undefined ? lite['اتجاه_س'] : 0, lite['اتجاه_ص'] !== undefined ? lite['اتجاه_ص'] : -1, lite['اتجاه_ع'] !== undefined ? lite['اتجاه_ع'] : 0);
                        const intensity = lite['قوة'] !== undefined ? lite['قوة'] : (lite['intensity'] !== undefined ? lite['intensity'] : 0.8);
                        const color = getColor(lite['لون'] || lite['color'] || '#ffffff');
                        
                        let babylonLight;
                        if (type.includes('منقطة') || type.includes('point')) {
                            babylonLight = new BABYLON.PointLight("light_" + lIdx, p, scene);
                        } else if (type.includes('موجهة') || type.includes('directional')) {
                            babylonLight = new BABYLON.DirectionalLight("light_" + lIdx, d, scene);
                            babylonLight.position = p;
                        } else {
                            babylonLight = new BABYLON.HemisphericLight("light_" + lIdx, d, scene);
                        }
                ...
                // دالة تمنح الكائنات خامات مخصصة وتلميحات بصرية متطورة
                const applyAdvancedMaterial = (mat, colorName) => {
                    const norm = (colorName || "").trim().toLowerCase();
                    mat.diffuseColor = getColor(colorName);
                    
                    if (norm.includes('عشب') || norm.includes('grass')) {
                        mat.specularColor = new BABYLON.Color3(0.05, 0.1, 0.05);
                        mat.roughness = 0.9;
                    } else if (norm.includes('ماء') || norm.includes('water') || norm.includes('بحر') || norm.includes('محيط')) {
                        mat.alpha = 0.65;
                        mat.specularPower = 120;
                        mat.specularColor = new BABYLON.Color3(0.9, 0.95, 1.0);
                    } else if (norm.includes('لافا') || norm.includes('lava') || norm.includes('بركان') || norm.includes('نار') || norm.includes('منصهر')) {
                        mat.emissiveColor = new BABYLON.Color3(0.95, 0.35, 0.05);
                        mat.specularColor = new BABYLON.Color3(0.2, 0.0, 0.0);
                    } else if (norm.includes('ذهب') || norm.includes('gold') || norm.includes('مذهب')) {
                        mat.diffuseColor = new BABYLON.Color3(0.95, 0.75, 0.15);
                        mat.specularColor = new BABYLON.Color3(1.0, 0.9, 0.6);
                        mat.specularPower = 90;
                        mat.roughness = 0.1;
                    } else if (norm.includes('خشب') || norm.includes('wood')) {
                        mat.specularColor = new BABYLON.Color3(0.1, 0.05, 0.02);
                        mat.roughness = 0.85;
                    } else if (norm.includes('حديد') || norm.includes('معدن') || norm.includes('metal') || norm.includes('steel')) {
                        mat.specularColor = new BABYLON.Color3(0.9, 0.9, 0.95);
                        mat.specularPower = 64;
                        mat.roughness = 0.2;
                    } else if (norm.includes('طوب') || norm.includes('brick')) {
                ...
                if (entitiesToRender.length > 0) {
                    ...
                    entitiesToRender.forEach((ent, i) => {
                        try {
                            ...
                            if (normType.includes('أرضية') || normType.includes('ارضية') || normType.includes('ground') || normType.includes('terrain') || normType.includes('مضمار') || normType.includes('منطقة')) {
                                ...
                                applyAdvancedMaterial(mat, colorName || '#1b2c1f');
                                mesh.material = mat;
                                ...
                                applyAdvancedMaterial(mat, colorName);
                                mesh.material = mat;
                                ...
                                applyAdvancedMaterial(mat, colorName || (normType.includes('وحش') || normType.includes('enemy') ? 'أحمر' : 'أخضر'));
                                mesh.material = mat;
                                ...
                                applyAdvancedMaterial(mat, colorName || 'أحمر');
                                mesh.material = mat;
                                ...
                                applyAdvancedMaterial(mat, colorName || 'رمادي');
                                mesh.material = mat;
                                mass = 0;
                            } else if (normType.includes('جزيئات') || normType.includes('particles') || normType.includes('لهب') || normType.includes('fire') || normType.includes('انفجار') || normType.includes('damp') || normType.includes('دخان')) {
                                mesh = BABYLON.MeshBuilder.CreateBox("emitter"+i, {size: 0.2}, scene);
                                mesh.isVisible = false;
                                mass = 0;
                                
                                const particleSystem = new BABYLON.ParticleSystem("particles" + i, 800, scene);
                                try {
                                    particleSystem.particleTexture = new BABYLON.Texture("https://raw.githubusercontent.com/PatrickRyanMS/BabylonJSTextures/master/ParticleDisks/sunParticle.png", scene);
                                } catch(pe) {}
                                particleSystem.emitter = mesh;
                                particleSystem.minEmitBox = new BABYLON.Vector3(-0.3, 0.0, -0.3);
                                particleSystem.maxEmitBox = new BABYLON.Vector3(0.3, 0.4, 0.3);
                                
                                const primaryColor = getColor(colorName || 'برتقالي');
                                particleSystem.color1 = new BABYLON.Color4(primaryColor.r, primaryColor.g, primaryColor.b, 1.0);
                                particleSystem.color2 = new BABYLON.Color4(primaryColor.r * 0.5, primaryColor.g * 0.1, 0, 0.3);
                                particleSystem.colorDead = new BABYLON.Color4(0.1, 0.1, 0.1, 0.0);
                                
                                particleSystem.minSize = ent['حجم_أصغر'] || ent['minSize'] || 0.1;
                                particleSystem.maxSize = ent['حجم_أكبر'] || ent['maxSize'] || 0.5;
                                particleSystem.minLifeTime = ent['عمر_أصغر'] || ent['minLife'] || 0.4;
                                particleSystem.maxLifeTime = ent['عمر_أكبر'] || ent['maxLife'] || 1.2;
                                particleSystem.emitRate = ent['تعداد'] || ent['emitRate'] || 120;
                                particleSystem.gravity = new BABYLON.Vector3(0, ent['جاذبية_جزيئات'] || 1.2, 0);
                    ...
                    if (playerMesh) {
                        const isCar = playerMesh.name.toLowerCase().includes("car") || playerMesh.name.toLowerCase().includes("body");
                        
                        if (camera.target === undefined || camera.target === null) {
                            camera.target = playerMesh;
                        }
                        
                        // If it's a follow camera, set target
                        if (camera.lockedTarget !== undefined) {
                        ...
                        scene.onBeforeRenderObservable.add(() => {
                            ...
                            if (isCar) {
                                ...
                                if (inputMap["w"] || inputMap["arrowup"]) thrust = carSpeed;
                                if (inputMap["s"] || inputMap["arrowdown"]) thrust = -carSpeed * 0.5;
                                if (inputMap["a"] || inputMap["arrowleft"]) torque = -0.04;
                                if (inputMap["d"] || inputMap["arrowright"]) torque = 0.04;
                                
                                if (playerMesh.physicsImpostor) {
                                    if (torque !== 0) {
                                        playerMesh.rotation.y += torque;
                                        playerMesh.physicsImpostor.setAngularVelocity(new BABYLON.Vector3(0, torque * 15, 0));
                                    } else {
                                        playerMesh.physicsImpostor.setAngularVelocity(new BABYLON.Vector3(0, 0, 0));
                                    }
                                    
                                    if (thrust !== 0) {
                                        const forward = playerMesh.getDirection(BABYLON.Axis.Z);
                                        const currentVel = playerMesh.physicsImpostor.getLinearVelocity();
                                        playerMesh.physicsImpostor.setLinearVelocity(new BABYLON.Vector3(forward.x * thrust, currentVel.y, forward.z * thrust));
                                    }
                                }
                            } else {
                                ...
                                if (playerMesh.physicsImpostor) {
                                    ...
                                    if (inputMap[" "] && Math.abs(curVel.y) < 0.15) {
